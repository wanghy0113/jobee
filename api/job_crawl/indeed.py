...
class IndeedCrawler(Crawler):
	...
	async def crawl(self, params: CrawlParams, callback: CrawlCallback, options = {}):
		...
		async with async_playwright() as p:
			...
			job_items = await job_items_container.query_selector_all('> li > div[class*="result"]')
			...
			for job_item in job_items:
				...
				job_info_container = await page.query_selector('div[class*="InfoHeaderContainer"]')
				...
				job_desp_container = await page.query_selector('div[id*="jobDescriptionText"]')
				if not job_desp_container:
					logging.debug(f'Job item {index} does not contain job description')
					continue
				job_desp = await job_desp_container.text_content()
